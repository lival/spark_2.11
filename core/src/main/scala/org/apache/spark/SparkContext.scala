...
class SparkContext(config: SparkConf) extends Logging {
  ...
  def runJob[T, U: ClassTag](
    ...
    /*val DebugStringInfo = rdd.toDebugString.toString
    println(DebugStringInfo)
    val outputFile = new File("G:/test.txt")
    try {
      val out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFile, true),"UTF-8"));
      out.write(DebugStringInfo + "\r\n");
      out.close()
    } catch {
