...
private[spark] object GradientBoostedTrees extends Logging {
  ...
  def boost(
    ...
    val baseLearners = new Array[DecisionTreeRegressionModel](numIterations) //每次迭代的决策树模型
    val baseLearnerWeights = new Array[Double](numIterations)   //每个树模型的权重
    val loss = boostingStrategy.loss //损失的计算方式
