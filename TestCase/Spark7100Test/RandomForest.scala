...
private[spark] object RandomForest extends Logging {
  ...
  def run(
    ...
    //建立决策树的元数据信息（分裂点位置、箱子数以及各箱子包含特征属性的值等）
    val metadata =
    ...
    //转换成树形的RDD类型，转换后，所有样本点分到各自切分点中，使得每棵树就是样本的一个子集
    val treeInput = TreePoint.convertToTreeRDD(retaggedInput, splits, metadata)

    val withReplacement = numTrees > 1  //多棵树时使用有放回抽样
//使用泊松分布对样本集进行抽样
    ...
    // Max memory usage for aggregates 计算聚合操作时节点的内存
    ...
    //节点是否使用缓存，节点ID从1开始，1即为这棵树的根节点，左节点为2，右节点为3，依次递增
     ...
     */
    //前面准备好数据，这里开始正式决策树的训练；初始化一个存储的结点栈
    ...
    //将（树的索引，数的根节点）入队，树索引从0开始，根节点从1开始，初始化每棵树的根节点
   ...
   //依次训练这numTrees颗树
    while (nodeStack.nonEmpty) {
      ...
      //RandomForest.findBestSplits从特征子集中选取最优的特征及特征值划分来构建树的结点，
      // 并在新建结点不是叶子结点时，将新的结点push入nodeStack栈，来实现循环并最终将树分裂至叶子结点。
